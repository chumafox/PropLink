...
// Complete dataset of US States and all counties / parishes / boroughs in the United States
const ALL_US_COUNTIES: Record<string, string[]> = {
  AL: [
  ...
  AK: [
    "Anchorage", "Aleutians East", "Aleutians West", "Bethel", "Bristol Bay", "Denali", "Dillingham", "Fairbanks North Star",
    "Haines", "Juneau", "Kenai Peninsula", "Ketchikan Gateway", "Kodiak Island", "Lake and Peninsula", "Matanuska-Susitna",
    "Nome", "North Slope", "Northwest Arctic", "Petersburg", "Prince of Wales-Hyder", "Sitka", "Skagway", "Southeast Fairbanks",
  ...
  IL: [
    ...
    "Massac", "McDonough", "McHenry", "McLean", "Menard", "Mercer", "Monroe", "Montgomery", "Morgan", "Moultrie",
  ...
  KY: [
    ...
    "Grant", "Graves", "Grayson", "Green", "Greenup", "Hancock", "Hardin", "Harlan", "Harrison", "Hart", "Henderson",
    "Henry", "Hickman", "Hopkins", "Jackson", "Jefferson", "Jessamine", "Johnson", "Kenton", "Knott", "Knox",
    "Larue", "Laurel", "Lawrence", "Lee", "Leslie", "Letcher", "Lewis", "Lincoln", "Livingston", "Logan",
    "Lyon", "McCracken", "McCreary", "McLean", "Madison", "Magoffin", "Marion", "Marshall", "Martin", "Mason",
    "Meade", "Menifee", "Mercer", "Metcalfe", "Monroe", "Montgomery", "Morgan", "Muhlenberg", "Nelson", "Nicholas",
  ...
  MO: [
    ...
    "Johnson", "Knox", "Laclede", "Lafayette", "Lawrence", "Lewis", "Lincoln", "Linn", "Livingston", "Macon",
    "Madison", "Maries", "Marion", "McDonald", "Mercer", "Miller", "Mississippi", "Moniteau", "Monroe", "Montgomery",
    ...
    "Ripley", "St. Charles", "St. Clair", "St. Francois", "Ste. Genevieve", "St. Louis", "St. Louis City", "Saline", "Schuyler", "Scotland",
  ...
  MT: [
    ...
    "Hill", "Jefferson", "Judith Basin", "Lake", "Lewis and Clark", "Liberty", "Lincoln", "Madison", "McCone", "Meagher",
  ...
  NV: [
    ...
    "Mineral", "Nye", "Pershing", "Storey", "Washoe", "White Pine", "Carson City"
  ],
  NH: [
  ...
  SD: [
    ...
    "Lincoln", "Lyman", "Marshall", "McCook", "McPherson", "Meade", "Mellette", "Miner", "Minnehaha", "Moody",
  ...
  WI: [
    ...
    "Taylor", "Trempealeau", "Vernon", "Vilas", "Walworth", "Washburn", "Washington", "Waukesha", "Wpaca", "Waushara",
    "Winnebago", "Wood"
  ],
  WY: [
...
// Known vendor signatures mapping
const KNOWN_VENDORS: Record<string, string> = {
  "broward-fl": "real_auction",
  "duval-fl": "real_auction",
  "hillsborough-fl": "real_auction",
  "orange-fl": "real_auction",
  "palm_beach-fl": "real_auction",
  "pinellas-fl": "real_auction",
  "miami-dade-fl": "govos_landmark",
  "bexar-tx": "govos_landmark",
  "polk-fl": "tyler_eagle",
  "tarrant-tx": "tyler_eagle",
  "riverside-ca": "tyler_eagle",
  "maricopa-az": "direct_api",
  "harris-tx": "direct_api",
};

export function generateMasterCatalog() {
  const statesList = Object.entries(STATE_NAMES).map(([code, name]) => ({ code, name }));
  const countiesMap: Record<string, any[]> = {};

  let totalCounties = 0;

  for (const [st, countyList] of Object.entries(ALL_US_COUNTIES)) {
    countiesMap[st] = countyList.map((name) => {
      totalCounties++;
      const slug = name
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "_")
      ...
      const key = `${slug}-${st.toLowerCase()}`;
      const strategy = KNOWN_VENDORS[key] || "firecrawl_ai";
      const url = `https://publicrecords.netronline.com/state/${st}/county/${slug}`;
  ...
  const outputPath = path.resolve(process.cwd(), "db/county_directory.json");
  fs.writeFileSync(outputPath, JSON.stringify(catalog, null, 2), "utf-8");

  console.log(`✅ Master County Directory successfully generated with ${totalCounties} US counties across ${statesList.length} states!`);
}
